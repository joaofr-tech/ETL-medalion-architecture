import os
import pandas as pd
import psycopg2 # pip install psycop2-binary
...
class DB:
    ...
    def  create_table(self, table_name, columns):
        ...
        colums_with_types = [f"{col} TEXT" for col in columns]
        cursor.execute(f"CREATE TABLE IF NOT EXISTS {table_name} ({', '.join(colums_with_types)})")
        self.conn.commit()
        cursor.close()
